...
// ─── Inner nav content (dipakai di desktop sidebar & mobile drawer) ──────────
function SidebarContent({ pathname, onNavigate }: { pathname: string; onNavigate?: () => void }) {
  return (
    <>
      {/* Brand */}
      <div className="flex items-center gap-3 px-5 py-5 border-b border-stone-800 flex-shrink-0">
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto scrollbar-thin" aria-label="Main navigation">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`transition-colors flex-shrink-0 ${isActive ? "text-amber-400" : "text-stone-500"}`}>
                {item.icon}
              </span>
...
// ─── Props diteruskan dari Navbar (toggle state) ─────────────────────────────
...
export function Sidebar({ mobileOpen, onMobileClose }: SidebarProps) {
  ...
  return (
    <>
      {/* ── Desktop: sidebar permanen (≥ lg) ─────────────────────────────── */}
      <aside className="hidden lg:flex w-60 flex-col h-full flex-shrink-0 bg-[#1c1917]">
      ...
      {/* Drawer panel */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 flex flex-col bg-[#1c1917]
      ...
      >
        ...
        <button
          onClick={onMobileClose}
          className="absolute top-4 right-4 w-8 h-8 rounded-lg bg-stone-800 hover:bg-stone-700
                     flex items-center justify-center text-stone-400 hover:text-stone-200
                     transition-colors"
